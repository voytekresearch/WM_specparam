...
FOOOF (specparam) twist. It imports exp2_EEGperCondition and performs a FOOOF analyses
...
from colourfulbirds.exp2_settings import (
    ...
    START_BAS, END_BAS, START_RET, END_RET, START_RET_CONTROL, END_RET_CONTROL, USE_UN_BASELINED, 
    SETS, FREQ_RANGE, SUB_THETA, SUB_NO_THETA
)
...
def get_bad_fits(df, frequency):
    ...
    if frequency == ALPHA:
        ...
        to_exclude_list = np.tile(to_exclude, 2*3) # 2 sides (ipsi/contra) ; 3 time frames
        # Add column to df
        ...
        # Boolean of trials that need to be excluded
        to_exclude_list = np.tile((exclude_bas + exclude_ret), 3) # 3 time frames
        # Add column to df
        df['to_exclude'] = to_exclude_list
...
def get_dfs(sub_n, performance, frequency, periodic_all, aperiodic_all, bandpower_all, auc_all, auc_log_all, auc_lin_all, percent_peak_all, r2s, lateralization=None):
    '''Create panda dataframes for parameters
    ...
    ----------
    df : pandas DataFrame
        Containing all the parameters for a single subject and performance
    psds : numpy 2D array
    ...
    col_names = ['sub_id', 'trial_n', 'performance', 'bas_ret_diff', 'exponent', 'offset', 'power',
    ...
    performance_list = np.repeat(performance, 3*n_trials)
    ...
    list_of_tuples = list(zip(sub_id_list, trial_list, performance_list, bas_ret_diff_list,
...
def get_spectrums(sub_eeg, timepoints, sfreq, timeframe, analysis_type): #, start_time=None, end_time=None, electrodes=None
    ...
    psd_av : np.array 2D
        Power spectra average over electrodes 
        of all trials from the EEG data without nan's [n_trials, 988 timepoints]
...
def get_fooof_params(sub_eeg, timepoints, timeframe, sfreq, frequency, sets, analysis_type): #start_time, end_time, 
    '''Similar to "get_fooof_params" function from experiment 1.
    Gives you the FOOOF parameters for either the baseline or retention period (timeframe).
    Baselining will be done in the function pad_baseline, only for the baseline period.
    Furthermore, if peaks aren't detected in the frequency range, the amplitude/power is nan and
    ...
    ----------
    periodic :
        Periodic parameters from fooof; amplitude/power, cf, bandwidth
    aperiodic :
        Aperiodic parameters from fooof; offset, exponent
    freq : np.array 1D
        Frequency axis corresponding to the PSD (psd_av)
    psd_av : np.array 2D
        Power spectra average over electrodes 
...
'''
    ...
    periodic = get_band_peak_fg(fg, frequency) # periodic params (cf, power, bandwidth)
    ...
    # Get bandpower
    bandpower = get_band_pow(freq, psd_av, frequency) # Not sure if this works with 2d psd structure
...
def get_fooof(sub_n, performance, sub_eeg, sub_acc, timepoints, frequency, sfreq, lateralization=None, analysis_type=None):
    """Function that creates PSD's in using get_spectrum(), and executes FOOOF fits using
    get_fooof_params() on both the baseline and retention period. Also, baselines the parameters
    per trial. Similar to "get_fooof" from experiment 1.
    ...
    performance : str
        Either GOOD or POOR
    sub_eeg :
        The eeg data processed from rejectBadTrials()
    sub_acc :
        The behavioral data processed from rejectBadTrials()
    frequency : (int, int)
        Frequency range of interest. Either Alpha or Theta
    sfreq : int
        Sampling frequency
    lateralization : str
        Either IPSI or CONTRA. Default is None which accounts for THETA

    Returns
    ----------
    periodic_dif_power : np.array 1D
        Demeaned/baselined periodic amplitude/power per trial
    periodic_dif_CF : np.array 1D
        Demeand/baselined periodic cf per trial
    aperiodic_dif : np.array 2D
        Demeaned/baselined aperiodic parameters (offset, exponent) per trial
    psd_av_bas : np.array
        Power spectrum of the baseline period
    psd_av_ret : np.array
        Power spectrum of the retention period
    freq_bas : np.array 1D
        Frequency axis corresponding to the PSD's
    """

    if frequency == ALPHA: # probably need to define this differently, since ALPHA is an array?
        eeg_data_left = get_eeg_data(sub_eeg, sub_acc, performance, LEFT, lateralization, eeg_channels=None)
        eeg_data_right = get_eeg_data(sub_eeg, sub_acc, performance, RIGHT, lateralization, eeg_channels=None)
        eeg_data = np.concatenate((eeg_data_left, eeg_data_right))
    else:  # frequency == THETA
        eeg_data_left = get_eeg_data(sub_eeg, sub_acc, performance, LEFT, eeg_channels=FRONTAL_CHANNELS)
        eeg_data_right = get_eeg_data(sub_eeg, sub_acc, performance, RIGHT, eeg_channels=FRONTAL_CHANNELS)
        eeg_data = np.concatenate((eeg_data_left, eeg_data_right))
    ...
    df = get_dfs(sub_n, performance, frequency, periodic_all, aperiodic_all, bp_all, auc_all, auc_log_all, auc_lin_all, perc_peak_all, r2s, lateralization)

    return df, psd_bas, psd_ret, freq_bas


def check_sub_fooof(sub_eeg, sfreq, sets, frequency):
    """Function to check if a subject has oscillations in a specific frequency range. Only used
    for ALPHA, since THETA is manually inspected). More specific; does the subject exhibit
    oscillatory activity over the entire trial?
    ...
    sets : dict
        Settings for creating the FOOOF object
    frequency : list
        list of 2 integers determining the lower an uppwer bound of the frequency band of interest

    Returns
    ----------
    FOOOFresults
        The results of the sepctral parameterization

    """

    # concatenate eeg_data
    eeg = np.concatenate((sub_eeg[0], sub_eeg[1]))

    # remove nan if present in the data
    if len(np.argwhere(np.isnan(eeg))) >= 1:
        nan = np.argwhere(np.isnan(eeg))
        eeg = np.delete(eeg, nan[0], axis = 0)

    # electrodes = get_freq_channels(frequency)
    electrodes = get_channels(check_sub=True, frequency=frequency)

    # Create PSD
    freq, psd = spectral.compute_spectrum(eeg[:, electrodes, :], sfreq, method='welch',
        avg_type='mean', nperseg=sfreq*2)
...
def check_sub_peak (eeg_data, frequency, sfreq, sets):
    ...
    for ALPHA, since THETA is manually inspected). More specific; does the subject exhibit
    oscillatory activity over the entire trial?
    If yes, then use this subject in further analyses. If no, reject this subject (in script).

    Parameters
    ----------
    eeg_data : ??
    frequency : (int, int)
        Frequency range of interest. Only used for ALPHA
    sfreq : int
        Sampling frequency
    sets : dict
        Settings for creating a FOOOF object
    ...
    fooof_data, model = check_sub_fooof(eeg_data, sfreq, sets, frequency)
    peak_data = fooof_data[1]

    peak_detect = peak_data[(peak_data[:,0] >= frequency[0]) & (peak_data[:,0] <= frequency[1])]

    if not peak_detect.shape[0]:  # if empty --> this is FALSE. Not makes it True if its empty
...
def pad_baseline(eeg):
    ...
    for trial in range(n_trial):
        for electrode in range(n_electrodes):
            # baseline the baseline period --> so that it's close to 0
            average = np.average(eeg[trial,electrode,:])
            baseline_period = eeg[trial, electrode, :] - average

            # add zero padding
            # half of the zeros on the left, half of the zeros on the right.
